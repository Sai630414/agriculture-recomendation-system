...
export class Sidebar {
  ...
  openLogin() {
    if (this.loginVisible) {
      // 🔁 Close login outlet
      this.router.navigate([{ outlets: { teluguloginOutlet: null } }], { relativeTo: this.route });
    } else {
      // ✅ Open login outlet
      this.router.navigate([{ outlets: { teluguloginOutlet: ['logintelugu'] } }], { relativeTo: this.route });
    }
